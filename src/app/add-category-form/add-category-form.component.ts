  ...
  async onSubmit(formValue: any) {
    if (this.addCategoryForm.valid) {
      ...
      (await this.task).ref.getDownloadURL().then((url: any) => {
        ...
        this.apiService.onboardCategoryAsParent(formValue).subscribe((response: any) => {
          const parentKey = response?.name;
          if (parentKey) {
            this.apiService.onboardCategoryForViewing(formValue, parentKey).subscribe(() => {
              this.isLoading = false;
              this.toastr.success('Machine Onboarded Successfully!', 'Close and refresh!', {
                timeOut: 4000,
                closeButton: true,
